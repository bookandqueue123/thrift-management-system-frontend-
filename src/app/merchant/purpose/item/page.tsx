...
import { PurposeProps, customer, roleResponse } from "@/types";
...
const Purpose = () => {
  ...
  const viewUser =
    ...
    (user?.role === "staff" &&
      userPermissions.includes(permissionsMap["view-purposes"]));

  const editPurpose =
    user?.role === "organisation" ||
    (user?.role === "staff" &&
      userPermissions.includes(permissionsMap["edit-purpose"]));
            ...
            >
              {modalContent === "form" ? (
                ...
                <ModalConfirmation
                  ...
                  status={
                    isPurposeCreated || isPurposeEdited ? "success" : "failed"
                  }
...
const MutateUser = ({
  ...
  useEffect(() => {
    if (actionToTake === "edit-purpose") {
      if (singlePurpose?.assignedCustomers) {
        const customerIds = singlePurpose.assignedCustomers?.map(
          (customer: { _id: any }) => customer,
        );
  ...
  const {
    data: allCustomersUnfiltered,
    isLoading: isLoadingAllCustomersUnfiltered,
  } = useQuery({
    queryKey: ["allCustomers"],
    queryFn: async () => {
      return client
  ...
  const validationSchema = Yup.object().shape({
    uniqueCode: Yup.string()
      .length(8, "The input must be exactly 8 characters")
      .optional(),
    purposeName: Yup.string().required("Purpose name is required"),
    description: Yup.string().required("Description is required"),
    category: Yup.string().required("Category is required"),
    amount: Yup.number().required("Amount is required"),
    amountWithoutCharge: Yup.number().required("Amount is required"),
    ...
    visibilityStartDate: Yup.date().required(
      "Visibility start date is required",
    ),
    visibilityStartTime: Yup.string().required(
      "Visibility start time is required",
    ),
  ...
  useEffect(() => {
    const calculateReferralBonusValue = () => {
      const { amount, amountWithoutCharge, referralBonus } = formik.values;
      const actualAmount =
        (organisation?.purposeCommission / 100) *
          formik.values.amountWithoutCharge +
        formik.values.amountWithoutCharge;
      const referralBonusValue = (referralBonus / 100) * amount;

      formik.setFieldValue("amount", actualAmount);
  ...
  const { mutate: createPurpose, isPending: isCreatingPurpose } = useMutation({
    mutationFn: async (values: any) => {
      ...
      formData.append("amountWithoutCharge", values.amountWithoutCharge);
      ...
      const assignedCustomers =
        values.visibility === "general"
          ? customerIds
          : values.assignedCustomers;

      assignedCustomers.forEach((item: string | Blob) =>
        formData.append("assignedCustomers[]", item),
      );
  ...
  return (
    <form onSubmit={formik.handleSubmit} className="space-y-4">
        ...
        <div className="flex flex-col space-y-2">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              />
              <label
                className="m-0 m-2 text-xs font-medium text-white"
                htmlFor="selectorAllOptional"
              >
                Optional for all
              </label>
            </div>
          ...
          <div className="flex items-center space-x-4">
            <div>
              <input
                ...
                onChange={formik.handleChange}
                checked={
                  formik.values.selectorCategory === "selectorCategoryMandatory"
                }
              />
              <label
                className="m-0 ml-2 text-xs font-medium text-white"
                htmlFor="selectorCategoryMandatory"
              >
                Mandatory for all
              </label>
            </div>
            <div>
              <input
                ...
                onChange={formik.handleChange}
                checked={
                  formik.values.selectorCategory === "selectorCategoryOptional"
                }
              />
              <label
                className="m-0 m-2 text-xs font-medium text-white"
      ...
      formik.values.visibility !== "general" ? (
        <div className="w-full">
          ...
          <div className="space-x-1 space-y-2">
            {formik.values.assignedCustomers.map(
      ...
      {formik.errors.assignedCustomers && formik.touched.assignedCustomers && (
        <div className="text-red-500">
          <>{formik.errors.assignedCustomers}</>
        </div>
      )}
      ...
      formik.values.visibility !== "general" ? (
        <div className="mt-2 flex  gap-x-3">
          <input
            ...
            checked={
              formik.values.assignedCustomers.length === allCustomers?.length
            }
            onChange={(e) => {
              if (e.target.checked) {
                formik.setFieldValue(
      ...
      {actionToTake === "edit-purpose" ? (
        <div className="flex gap-x-3">
          <input
            ...
            type="checkbox"
            className="block h-4 w-4 rounded-md border-gray-300 text-indigo-600 focus:ring-indigo-600"
            checked={assignedCustomerIds.length === allCustomers?.length}
            onChange={(e) => {
              if (e.target.checked) {
                if (allCustomers) {
                  setAssignedCustomerIds(
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="flex flex-col space-y-2">
          ...
          >
            ...
            {formik.values.visibility === "inhouse"
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="flex flex-col space-y-2 ">
          <label
            className="m-0 text-xs font-medium text-white"
            htmlFor="amountWithoutCharge"
          >
            Amount
            <span className="font-base font-semibold text-[#FF0000]">*</span>
          </label>
          <input
            id="amountWithoutCharge"
            name="amountWithoutCharge"
            type="number"
            onChange={formik.handleChange}
            value={formik.values.amountWithoutCharge}
            className="bg-right-20 w-full rounded-lg border-0  bg-[#F3F4F6] bg-[url('../../public/arrow_down.svg')] bg-[95%_center] bg-no-repeat p-3 text-[#7D7D7D] md:bg-none"
          />
          {formik.errors.amountWithoutCharge &&
            formik.touched.amountWithoutCharge && (
              <div className="text-red-500">
                <>{formik.errors.amountWithoutCharge}</>
              </div>
            )}
        ...
        <div className="flex flex-col space-y-2 ">
          ...
          <input
            disabled
            id="platformservicecharge"
            name="platformservicecharge"
            type="number"
            onChange={formik.handleChange}
            value={organisation?.purposeCommission}
            className="bg-right-20 w-full rounded-lg border-0  bg-[#F3F4F6] bg-[url('../../public/arrow_down.svg')] bg-[95%_center] bg-no-repeat p-3 text-[#7D7D7D] md:bg-none"
          />
          {/* {formik.errors.platformservicecharge &&
            formik.touched.platformservicecharge && (
              <div className="text-red-500">
                <>{formik.errors.platformservicecharge}</>
              </div>
            )} */}
      ...
      <div>
        <h1 className="text-xl text-white">Maximum payment Duration</h1>
      ...
      <div className="grid grid-cols-4 gap-4">
        <div className="flex flex-col space-y-2">
          ...
          <div className="relative flex flex-col items-center sm:flex-row">
            <button
              ...
              className="btn mb-2 rounded-md bg-blue-500 px-2 text-xs text-white sm:absolute sm:left-2 sm:top-1/2 sm:mb-0 sm:-translate-y-1/2 sm:transform md:px-4 md:text-lg"
            >
              Generate
            </button>
            <input
              ...
              className="w-full rounded-lg border-0 bg-[#F3F4F6] p-3 pl-4 text-[#7D7D7D] sm:pl-[7rem]"
            />
          </div>
        </div>
        ...
        <div className="flex flex-col space-y-2">
          ...
          <input
            ...
            className="bg-right-20 w-full rounded-lg border-0  bg-[#F3F4F6] bg-[url('../../public/arrow_down.svg')] bg-[95%_center] bg-no-repeat p-3 text-[#7D7D7D] md:bg-none"
          />
          {formik.errors.referralBonusValue &&
            formik.touched.referralBonusValue && (
              <div className="text-red-500">
                <>{formik.errors.referralBonusValue}</>
              </div>
            )}
          ...
          >
            ...
            <div className="flex flex-col items-center justify-center">
              ...
              <p className="text-center text-[gray]">
                Drag n drop a <span className="font-semibold">.jpg, .png</span>{" "}
      ...
      {actionToTake === "edit-purpose" ? (
        <div className="mb-8">
          ...
          <div className="mt-8">
            ...
            {formik.isSubmitting && <span className="ml-2">Uploading...</span>}
          </div>
        </div>
      ) : (
        ""
      ...
      {actionToTake === "create-purpose" ? (
        <div className="mt-4">
          ...
          {formik.values.digitalItem &&
            ...
            ((formik.values.digitalItem[0] as unknown as File).type.includes(
              "image",
            ) ? (
              <Image
      ...
      <div className="grid grid-cols-4 gap-4">
        <div className="flex flex-col space-y-2">
          ...
          <input
            ...
            className="bg-right-20 w-full rounded-lg border-0 bg-[#F3F4F6] bg-[url('../../public/arrow_down.svg')] bg-[95%_center] bg-no-repeat p-3 text-[#7D7D7D] md:bg-none"
          />
          {formik.errors.visibilityStartDate &&
            formik.touched.visibilityStartDate && (
              <div className="text-red-500">
                {formik.errors.visibilityStartDate}
              </div>
            )}
        ...
        <div className="flex flex-col space-y-2">
          ...
          <input
            ...
            className="bg-right-20 w-full rounded-lg border-0  bg-[#F3F4F6] bg-[url('../../public/arrow_down.svg')] bg-[95%_center] bg-no-repeat p-3 text-[#7D7D7D] md:bg-none"
          />
          {formik.errors.visibilityStartTime &&
            formik.touched.visibilityStartTime && (
              <div className="text-red-500">
                <>{formik.errors.visibilityStartTime}</>
              </div>
            )}
        ...
        <div className="flex flex-col space-y-2">
          ...
          <input
            ...
            className="bg-right-20 w-full rounded-lg border-0 bg-[#F3F4F6] bg-[url('../../public/arrow_down.svg')] bg-[95%_center] bg-no-repeat p-3 text-[#7D7D7D] md:bg-none"
          />
          {formik.errors.visibilityEndDate &&
            formik.touched.visibilityEndDate && (
              <div className="text-red-500">
                {formik.errors.visibilityEndDate}
              </div>
            )}
        ...
        <div className="flex flex-col space-y-2">
          ...
          <input
            ...
            className="bg-right-20 w-full rounded-lg border-0  bg-[#F3F4F6] bg-[url('../../public/arrow_down.svg')] bg-[95%_center] bg-no-repeat p-3 text-[#7D7D7D] md:bg-none"
          />
          {formik.errors.visibilityEndTime &&
            formik.touched.visibilityEndTime && (
              <div className="text-red-500">
                <>{formik.errors.visibilityEndTime}</>
              </div>
            )}
