...
const DeliveryPage = () => {
  ...
  // Get all states
  const states = statesAndLGAs[0]?.states || []
  const cities = selectedState ? states.find((s: any) => s.name === selectedState)?.lgas || [] : []
  ...
  const handleViewTerms = async (type: "door" | "pickup") => {
    if (cartItems.length === 0) {
      setModalContent({
        title: type === "door" ? "Door Delivery Terms and Conditions" : "Pickup Terms and Conditions",
    ...
    const cartProducts = allProducts.filter((product) => productIds.includes(product._id))
    ...
    cartProducts.forEach((product, index) => {
      const terms = type === "door" ? product.doorDeliveryTermsAndCondition : product.pickupCentreTermsAndCondition
      ...
      <div className="min-h-screen bg-gray-50 pb-12">
        ...
        <div className="max-w-2xl mx-auto px-4 py-8">
          ...
          <div className="space-y-4 bg-white p-6 rounded-lg shadow border">
            ...
            <div>
              <label className="block text-sm font-medium mb-1">Choose Your Preferred Mode Of Delivery</label>
              <div className="space-y-4">
                <div
                  className={`p-4 rounded border cursor-pointer ${deliveryMode === "door" ? "border-orange-500 bg-orange-50" : "border-gray-300"}`}
                  onClick={() => setDeliveryMode("door")}
                >
                  ...
                  <button
                    ...
                    className="text-sm text-blue-600 hover:underline ml-7 disabled:opacity-50"
                    disabled={isLoadingProducts}
                  >
                    {isLoadingProducts ? "Loading terms..." : "View terms and conditions"}
                  </button>
                </div>
            ...
            {deliveryMode === "pickup" && (
              <div>
                ...
                {showAvailableOnly ? (
                  ...
                  <div className="text-gray-500 text-sm">Loading centers...</div>
                ) : (
                  <select
                    className="w-full border rounded p-2"
                    value={selectedPickup}
      ...
      {showTermsModal && (
        <Modal setModalState={setShowTermsModal} title={modalContent.title}>
          <div className=" rounded-lg p-6 max-h-96 overflow-y-auto">
            <div className="whitespace-pre-wrap text-white  leading-relaxed font-medium">{modalContent.content}</div>
          </div>
        </Modal>
      )}
    </>
