...
//       await client.post("/api/cart", { productId, quantity });
//     } catch (error) {
    ...
    >
      ...
      <div className="relative mb-4">
        <div className={`flex h-48 w-full items-center justify-center overflow-hidden rounded-lg bg-gray-100 ${
          isOutOfStock ? "" : "transition-opacity hover:opacity-90"
        }`}>
          {product.imageUrl ? (
            <img
              src={
        ...
        <button 
          ...
          className="absolute right-2 top-2 z-20 rounded-full bg-white p-1 shadow-md hover:bg-gray-50"
        >
          <Heart className="h-4 w-4 text-gray-400" />
        </button>
      </div>
      ...
      <div>
        ...
        <div className="mb-2 flex items-center gap-2">
          <span className={`text-lg font-bold ${
            isOutOfStock ? "text-gray-500" : "text-orange-600"
          }`}>
            ₦{product.price.toLocaleString()}
          </span>
