...
const Create = () => {
  ...
  const createProductMutation = useMutation({
    mutationFn: async (data: ProductFormData) => {
      ...
      form.append("originalPrice", sanitizedData.costBeforeDiscount);
  ...
  const openProductModal = (id: string, type: "view" | "edit") => {
    ...
    if (type === "edit" && products) {
      ...
      if (productToEdit) {
        ...
        const platformPercent = platformChargeData?.data?.percentage || 0;

        // CORRECTED: Calculate platform charge value
        const platformChargeVal =
          (Number(platformPercent) / 100) * Number(productToEdit.originalPrice);
        setPlatformChargeValue(platformChargeVal.toFixed(2));

        // CORRECTED: Calculate actual price
        const actualPriceVal =
          Number(productToEdit.originalPrice) + platformChargeVal;
        setActualPrice(actualPriceVal.toFixed(2));

        setFormData({
          name: productToEdit.name,
          description: productToEdit.description,
          price: String(productToEdit.price),
          costBeforeDiscount: String(productToEdit.originalPrice),
            ...
            <div className="space-y-4 rounded-lg border border-gray-700 p-6">
              ...
              <div>
                ...
                <div className="w-full rounded border bg-gray-200 px-3 py-2 text-black">
                  ₦{" "}
                  {AmountFormatter(
                    singleProduct.costBeforeDiscount +
                      (platformChargeData?.data?.percentage
                        ? (singleProduct.costBeforeDiscount *
                            platformChargeData.data.percentage) /
                          100
              ...
              <div>
                ...
                <div className="w-full rounded border bg-gray-200 px-3 py-2 text-black">
                  {(singleProduct as any).mininumDepositPercentage &&
                  singleProduct.price
                    ? `₦ ${AmountFormatter((parseFloat((singleProduct as any).mininumDepositPercentage) / 100) * singleProduct.price)}`
                    : "-"}
                </div>
              </div>
              <div>
              ...
              <div>
                <label className="block text-white">
                  Storage Outdoor (H x L x B)
                </label>
                <div className="w-full rounded border bg-gray-200 px-3 py-2 text-black">
                  {(singleProduct as any).storageOutdoor
                    ? `${(singleProduct as any).storageOutdoor.height || "-"} x ${(singleProduct as any).storageOutdoor.length || "-"} x ${(singleProduct as any).storageOutdoor.breadth || "-"}`
                    : "-"}
                </div>
              </div>
              <div>
                <label className="block text-white">
                  Storage Refrigerated (H x L x B)
                </label>
                <div className="w-full rounded border bg-gray-200 px-3 py-2 text-black">
                  {(singleProduct as any).storageRefrigerated
                    ? `${(singleProduct as any).storageRefrigerated.height || "-"} x ${(singleProduct as any).storageRefrigerated.length || "-"} x ${(singleProduct as any).storageRefrigerated.breadth || "-"}`
                    : "-"}
                </div>
              </div>
              <div>
