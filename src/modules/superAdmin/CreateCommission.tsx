...
const CreateCommissionForm: React.FC = () => {
  ...
  const {mutate: setUpCommssion, isPending: settingUpCommission, isError } = useMutation({
    mutationKey: ["Setup Commisson"],
    mutationFn: async() => {
      let payload
      if(formValues.organisationType === 'groupOfOrganisation'){
        payload = {
          commission: formValues.applyCommission,
      ...
      {formValues.organisationType === 'groupOfOrganisation' && (
        <div className='items-center gap-6 md:flex'>
          ...
          <span className='w-full'>
            ...
            {organizationsGroups?.map((group: OrganisationGroupsProps, index: number) => (
              <option value={group._id} key={group._id}>{group.groupName}</option>
        ...
        <div className='items-center gap-6 md:flex mt-4'>
          ...
          <span className='w-full'>
            <select name="organisation" onChange={handleInputChange} className="bg-right-20 mt-1 w-full cursor-pointer appearance-none  rounded-lg border-0 bg-[#F3F4F6] bg-[url('../../public/arrow_down.svg')] bg-[95%_center] bg-no-repeat p-3 text-[#7D7D7D]">
                ...
                {organizations?.map((organization: getOrganizationProps, index:number) =>(
                  <option key={organization._id} value={organization._id}>{organization.organisationName}</option>
                ))}
            </select> 
